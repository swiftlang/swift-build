...
private class SettingsBuilder: ProjectMatchLookup {
    ...
    func construct() -> (BoundProperties, BoundDeploymentTarget) {
        ...
        if scope.evaluate(BuiltinMacros.ENABLE_MEMORY_TAGGING_ADDRESS_SANITIZER) {
            let supportedArchs = Set(["arm64", "arm64e"])
            let buildArchs = Set(scope.evaluate(BuiltinMacros.ARCHS))
            let unsupportedArchs = buildArchs.subtracting(supportedArchs)

            if !unsupportedArchs.isEmpty {
                let archList = unsupportedArchs.sorted().joined(separator: ", ")
                self.errors.append("Memory-Tagging Address Sanitizer is enabled (ENABLE_MEMORY_TAGGING_ADDRESS_SANITIZER = YES), but is not supported for architecture(s): \(archList). Memory-Tagging Address Sanitizer is only supported on arm64 architectures.")
            }
        }
