...
public struct DiscoveredLdLinkerToolSpecInfo: DiscoveredCommandLineToolSpecInfo {
    ...
    public func supportsTraceFile() -> Bool {
        #if os(macOS)
        return (try? toolVersion >= Version("1162.191")) ?? false
        #else
        return false
        #endif
...
public final class LdLinkerSpec : GenericLinkerSpec, SpecIdentifierType, @unchecked Sendable {
    ...
    override public func constructLinkerTasks(_ cbc: CommandBuildContext, _ delegate: any TaskGenerationDelegate, libraries: [LibrarySpecifier], usedTools: [CommandLineToolSpec: Set<FileTypeSpec>]) async {
        ...
        let ldOptionContext = optionContext as? DiscoveredLdLinkerToolSpecInfo
        ...
        for info in usedTools.sorted(byKey: \.identifier) {
            ...
            if let linker = ldOptionContext?.linker, linker != .ld64 && machOTypeString == "mh_object" {
                continue
            }
        ...
        inputs.append(contentsOf: additionalInputDependencies(cbc, delegate, optionContext: optionContext, lookup: lookup).map(delegate.createNode))
        ...
        let supportsTraceFile = !usesLDClassic && ldOptionContext?.supportsTraceFile() ?? false
        let taskDependencySettings: TaskDependencySettings? = if supportsTraceFile {
            TaskDependencySettings(
...
public func discoveredLinkerToolsInfo(_ producer: any CommandProducer, _ delegate: any CoreClientTargetDiagnosticProducingDelegate, at toolPath: Path) async -> DiscoveredLdLinkerToolSpecInfo? {
    do {
        do {
