...
public protocol BuildFileFilteringContext: PlatformFilteringContext, BuildConfigurationFilteringContext {
...
public enum BuildFileExclusionReason {
    ...
    /// The file was excluded because its build configuration filters did not match that of the current build context.
...
extension BuildFileFilteringContext {
    ...
    public func filterState(of path: Path, platformFilters: Set<PlatformFilter>, buildConfigurationFilters: Set<BuildConfigurationFilter>) -> BuildFileFilterState {
        if !currentPlatformFilter.matches(platformFilters) {
            return .excluded(.platformFilter)
        }

        if !currentBuildConfigurationFilter.matches(buildConfigurationFilters) {
            return .excluded(.buildConfigurationFilter)
        }
    ...
    /// Returns whether `path` should be excluded from the build phase based on the build settings.  See `filterState(of:platformFilters:buildConfigurationFilters)` for details.
    public func isExcluded(_ path: Path, platformFilters: Set<PlatformFilter>, buildConfigurationFilters: Set<BuildConfigurationFilter>) -> Bool {
        switch filterState(of: path, platformFilters: platformFilters, buildConfigurationFilters: buildConfigurationFilters) {
        case .excluded:
            return true
        case .included:
            return false
