...
public final class LdLinkerSpec : GenericLinkerSpec, SpecIdentifierType {
    ...
    static public func computeRPaths(_ cbc: CommandBuildContext,_ delegate: any TaskGenerationDelegate, inputRunpathSearchPaths: [String], isUsingSwift: Bool ) async -> [String] {
        ...
        if (cbc.producer.platform?.supportsSwiftInTheOS(cbc.scope, forceNextMajorVersion: true, considerTargetDeviceOSVersion: false) != true || cbc.producer.toolchains.usesSwiftOpenSourceToolchain || shouldEmitRPathForSwiftConcurrency) && isUsingSwift && cbc.producer.platform?.identifier != "webassembly" {
    ...
    private func generateThunkPreviewInfo(for task: any ExecutableTask, sourceFile: Path, thunkVariantSuffix: String) -> [TaskGeneratePreviewInfoOutput] {
        ...
        for arg in ["-dynamiclib", "-bundle", "-r", "-fembed-bitcode-marker", "-dead_strip", "-nostdlib"] {
