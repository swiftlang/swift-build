...
package final class SourcesTaskProducer: FilesBasedBuildPhaseTaskProducerBase, FilesBasedBuildPhaseTaskProducer {
    ...
    func computeLibraries(_ buildFilesContext: BuildFilesProcessingContext, _ scope: MacroEvaluationScope) -> [LinkerSpec.LibrarySpecifier] {
        ...
        return buildFiles.compactMap { buildFile -> LinkerSpec.LibrarySpecifier? in
            ...
            // On Windows, always use search paths to ensure import libraries (.lib) are found correctly instead of DLLs.
            ...
            case .targetProduct(guid: let guid):
                if let referenceTarget = self.context.workspaceContext.workspace.target(for: guid) {
                    ...
                    // Always use search paths on Windows to ensure .lib files are found instead of .dll files
                    if context.sdkVariant?.llvmTargetTripleSys == "windows" {
                        useSearchPaths = true
                    } else {
