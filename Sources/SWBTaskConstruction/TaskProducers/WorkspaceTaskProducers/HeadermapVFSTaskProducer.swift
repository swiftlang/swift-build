...
extension TaskProducerContext {
    func constructVFSContents() async throws -> ByteString {
        ...
        try await forEachBuildProductToSourceMapping(mapToTempFilesOnly: true) { target, isActiveTarget, builtFile, originalSourceFile in
            if isActiveTarget {
    ...
    func forEachBuildProductToSourceMapping(mapToTempFilesOnly: Bool, _ body: ((target: SWBCore.Target, isActiveTarget: Bool, builtFile: Path, originalSource: Path)) throws -> Void) async throws {

        let index = try await workspaceContext.headerIndex
