...
public final class ProcessController: Sendable {
    ...
    /// - parameter input: The file descriptor to read from for standard input.
    /// - parameter output: The file descriptor to write to for standard output.
    /// - parameter error: The file descriptor to write to for standard error.
    /// - parameter highPriority: Whether to increase the process's priority in a platform-specific way (quality of service on Darwin).
    /// - parameter onStarted: Called with the running process's PID once the process has started running, or `nil` if the process did not start successfully. Will be called exactly once.
    public func start(input: FileDescriptor, output: FileDescriptor, error: FileDescriptor, highPriority: Bool, onStarted: @Sendable @escaping (_ pid: pid_t?) -> ()) {
            ...
            let task = Task<Void, Never>.detached { [path, arguments, environment, workingDirectory, done] in
                ...
                let result = await Result<Processes.ExitStatus, any Error>.catching {
                    ...
                    let configuration = Subprocess.Configuration(
                        ...
                        environment: environment.map { .custom([String: String]($0)) } ?? .inherit,
                ...
                let invokedOnStarted = self.state.withLock { state in
                    switch state {
                    case let .running(runningState):
                        state = .exited(exitStatus: result)
                        return runningState.pid != nil
