...
public actor SWBBuildServer: QueueBasedMessageHandler {
    ...
    private func prepare(request: BuildTargetPrepareRequest) async throws -> SWBBuildServerProtocol.VoidResponse {
    ...
    private func reportEventStream(_ events: AsyncStream<SwiftBuildMessage>) async {
        let buildTaskID = UUID().uuidString
        for try await event in events {
            switch event {
            case .planningOperationStarted(let info):
                logToClient(.log, "Planning Build", .begin(.init(title: "Planning Build", taskID: info.planningOperationID)))
            case .planningOperationCompleted(let info):
                logToClient(.info, "Build Planning Complete", .end(.init(taskID: info.planningOperationID)))
            case .buildStarted(_):
                logToClient(.log, "Building", .begin(.init(title: "Building", taskID: buildTaskID)))
            case .buildDiagnostic(let info):
                logToClient(.log, info.message, .report(.init(taskID: buildTaskID)))
            case .buildCompleted(let info):
                switch info.result {
                case .ok:
                    logToClient(.log, "Build Complete", .end(.init(taskID: buildTaskID)))
                case .failed:
                    logToClient(.log, "Build Failed", .end(.init(taskID: buildTaskID)))
                case .cancelled:
                    logToClient(.log, "Build Cancelled", .end(.init(taskID: buildTaskID)))
                case .aborted:
                    logToClient(.log, "Build Aborted", .end(.init(taskID: buildTaskID)))
                }
            case .preparationComplete(_):
                logToClient(.log, "Build Preparation Complete", .end(.init(taskID: buildTaskID)))
            case .didUpdateProgress(let info):
                logToClient(.log, "Progress: \(info.percentComplete)%", .end(.init(taskID: buildTaskID)))
            case .taskStarted(let info):
                logToClient(.log, info.executionDescription, .begin(.init(title: info.executionDescription, taskID: "\(buildTaskID)-task-\(info.taskID)")))
            case .taskDiagnostic(let info):
                logToClient(.log, info.message, .report(.init(taskID: "\(buildTaskID)-task-\(info.taskID)")))
            case .taskComplete(let info):
                logToClient(.log, "Task Complete", .end(.init(taskID: "\(buildTaskID)-task-\(info.taskID)")))
    ...
    private func logToClient(_ kind: BuildServerMessageType, _ message: String, _ structure: StructuredLogKind? = nil) {
    ...
    private func logTaskToClient<T>(name: String, _ perform: (String) async throws -> T) async throws -> T {
        ...
        logToClient(.log, name, .begin(.init(title: name, taskID: taskID)))
        defer {
            logToClient(.log, name, .end(.init(taskID: taskID)))
        }
