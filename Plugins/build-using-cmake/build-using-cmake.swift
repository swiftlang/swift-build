...
struct RunXcodebuild: CommandPlugin {
    func performCommand(context: PluginContext, arguments: [String]) async throws {
        ...
        for url in [swiftToolsSupportCoreBuildURL, swiftSystemURL, llbuildBuildURL, swiftArgumentParserBuildURL, swiftDriverBuildURL, swiftBuildBuildURL] {
            try FileManager.default.createDirectory(at: url, withIntermediateDirectories: true)
        }

        print("Building swift-tools-support-core")
        try await Process.checkNonZeroExit(url: cmakeURL, arguments: ["-G", "Ninja", "-DCMAKE_MAKE_PROGRAM=\(ninjaPath)", "-DCMAKE_BUILD_TYPE:=Debug", "-DCMAKE_Swift_FLAGS='-sdk \(sysrootPath)'", swiftToolsSupportCoreURL.path()], workingDirectory: swiftToolsSupportCoreBuildURL)
        ...
        try await Process.checkNonZeroExit(url: cmakeURL, arguments: ["-G", "Ninja", "-DCMAKE_MAKE_PROGRAM=\(ninjaPath)", "-DCMAKE_BUILD_TYPE:=Debug", "-DCMAKE_Swift_FLAGS='-sdk \(sysrootPath)'", swiftSystemURL.path()], workingDirectory: swiftSystemBuildURL)
        ...
        try await Process.checkNonZeroExit(url: cmakeURL, arguments: ["-G", "Ninja", "-DCMAKE_MAKE_PROGRAM=\(ninjaPath)", "-DCMAKE_BUILD_TYPE:=Debug", "-DCMAKE_Swift_FLAGS='-sdk \(sysrootPath)'", "-DLLBUILD_SUPPORT_BINDINGS:=Swift", llbuildURL.path()], workingDirectory: llbuildBuildURL)
        ...
        try await Process.checkNonZeroExit(url: cmakeURL, arguments: ["-G", "Ninja", "-DCMAKE_MAKE_PROGRAM=\(ninjaPath)", "-DCMAKE_BUILD_TYPE:=Debug", "-DCMAKE_Swift_FLAGS='-sdk \(sysrootPath)'", "-DBUILD_TESTING=NO", "-DBUILD_EXAMPLES=NO", swiftArgumentParserURL.path()], workingDirectory: swiftArgumentParserBuildURL)
        ...
        try await Process.checkNonZeroExit(url: cmakeURL, arguments: ["-G", "Ninja", "-DCMAKE_MAKE_PROGRAM=\(ninjaPath)", "-DCMAKE_BUILD_TYPE:=Debug", "-DCMAKE_Swift_FLAGS='-sdk \(sysrootPath)'", "-DArgumentParser_DIR=\(swiftArgumentParserBuildURL.appending(components: "cmake", "modules").path())", "-DLLBuild_DIR=\(llbuildBuildURL.appending(components: "cmake", "modules").path())", "-DTSC_DIR=\(swiftToolsSupportCoreBuildURL.appending(components: "cmake", "modules").path())", swiftDriverURL.path()], workingDirectory: swiftDriverBuildURL)
        ...
        try await Process.checkNonZeroExit(url: cmakeURL, arguments: ["-G", "Ninja", "-DCMAKE_MAKE_PROGRAM=\(ninjaPath)", "-DCMAKE_BUILD_TYPE:=Debug", "-DCMAKE_Swift_FLAGS='-sdk \(sysrootPath)'", "-DArgumentParser_DIR=\(swiftArgumentParserBuildURL.appending(components: "cmake", "modules").path())", "-DLLBuild_DIR=\(llbuildBuildURL.appending(components: "cmake", "modules").path())", "-DTSC_DIR=\(swiftToolsSupportCoreBuildURL.appending(components: "cmake", "modules").path())", "-DSwiftDriver_DIR=\(swiftDriverBuildURL.appending(components: "cmake", "modules").path())", "-DSwiftSystem_DIR=\(swiftSystemBuildURL.appending(components: "cmake", "modules").path())", swiftBuildURL.path()], workingDirectory: swiftBuildBuildURL)
