...
@Suite fileprivate struct SDKRegistryTests: CoreBasedTests {
    ...
    /// Checks that our "missing frameworks" lists are accurate. This is dependent on the installed SDK version but tries to balance test coverage against maintainability by checking specific frameworks whose SDK/platform availability is unlikely to change.
    @Test(.requireSDKs(.macOS, .iOS, .tvOS, .watchOS))
    func missingFrameworks() async throws {
        let realRegistry = try await getCore().sdkRegistry

        let macosxSDK = try #require(realRegistry.lookup("macosx"))
        let macCatalystSDKVariant = try #require(macosxSDK.variants[MacCatalystInfo.sdkVariantName])
        let iphoneosSDK = try #require(realRegistry.lookup("iphoneos"))
        let appletvosSDK = try #require(realRegistry.lookup("appletvos"))
        let watchosSDK = try #require(realRegistry.lookup("watchos"))

        let macosxSDKUnavailable = realRegistry.knownUnavailableFrameworksForSDK(macosxSDK, sdkVariant: nil).sorted()
        let macCatalystSDKUnavailable = realRegistry.knownUnavailableFrameworksForSDK(macosxSDK, sdkVariant: macCatalystSDKVariant).sorted()
        let iosSDKUnavailable = realRegistry.knownUnavailableFrameworksForSDK(iphoneosSDK, sdkVariant: nil).sorted()
        let tvosSDKUnavailable = realRegistry.knownUnavailableFrameworksForSDK(appletvosSDK, sdkVariant: nil).sorted()
        let watchosSDKUnavailable = realRegistry.knownUnavailableFrameworksForSDK(watchosSDK, sdkVariant: nil).sorted()
        #expect(macosxSDKUnavailable != macCatalystSDKUnavailable)

        // Unavailable in macOS (but potentially available on any other platform)
        XCTAssertSuperset(Set(macosxSDKUnavailable), ["MobileCoreServices", "OpenGLES", "UIKit", "TVUIKit", "WatchKit"])

        // Unavailable in macCatalyst (but potentially available on any other platform)
        XCTAssertSuperset(Set(macCatalystSDKUnavailable), ["Carbon", "Cocoa", "DriverKit", "Hypervisor", "SystemExtensions", "TVUIKit", "WatchKit"])

        // Unavailable in macCatalyst but were ALSO available in iOS
        // These are the ones the macCatalyst opt-in workflow should conditionalize.

        let macCatalystSDKUnavailableIosAvailable = Set(macCatalystSDKUnavailable).subtracting(iosSDKUnavailable)
        XCTAssertSuperset(macCatalystSDKUnavailableIosAvailable, ["GLKit", "OpenGLES"])

        // Do a less stringent check on the iOS/tvOS/watchOS SDK for now to reduce maintenance burden.
        // For now we'll check that there aren't more frameworks than expected by a rough margin of error of 20.
        #expect(iosSDKUnavailable.count < 130) // ~84
        #expect(tvosSDKUnavailable.count < 230) // ~180
        #expect(watchosSDKUnavailable.count < 240) // ~210

        XCTAssertSuperset(Set(iosSDKUnavailable), ["AppKit", "TVUIKit"])
        XCTAssertSuperset(Set(tvosSDKUnavailable), ["AppKit", "WatchKit"])
        XCTAssertSuperset(Set(watchosSDKUnavailable), ["AppKit", "TVUIKit"])
    }

    @Test(.requireSDKs(.macOS, .iOS, .tvOS, .watchOS))
    func knownFrameworks() async throws {
        let realRegistry = try await getCore().sdkRegistry

        let macosxSDK = try #require(realRegistry.lookup("macosx"))
        let iphoneosSDK = try #require(realRegistry.lookup("iphoneos"))

        let macOSKnownFrameworks = macosxSDK.knownFrameworkNames(for: macosxSDK.variant(for: "macos"))
        let macCatalystKnownFrameworks = macosxSDK.knownFrameworkNames(for: macosxSDK.variant(for: MacCatalystInfo.sdkVariantName))
        let iOSKnownFrameworks = iphoneosSDK.knownFrameworkNames(for: nil)

        // macOS and macCatalyst shouldn't be equal, because the latter should have more...
        #expect(macOSKnownFrameworks != macCatalystKnownFrameworks)

        // ...and macCatalyst should NOT always contain everything macOS does because it isn't a superset -- some macOS frameworks are unavailable to macCatalyst
        #expect(!macCatalystKnownFrameworks.isSuperset(of: macOSKnownFrameworks))

        // correctness checks
        #expect(macosxSDK.knownFrameworkNames != macOSKnownFrameworks)
        #expect(macosxSDK.knownFrameworkNames != macosxSDK.knownFrameworkNames(for: nil))
        #expect(macosxSDK.knownFrameworkNames != macCatalystKnownFrameworks)

        // Shouldn't be an observable difference for other platforms
        #expect(iphoneosSDK.knownFrameworkNames == iOSKnownFrameworks)

        // Neither iOS nor macCatalyst are likely to have available framework lists which are supersets of each other
        #expect(!iOSKnownFrameworks.isSuperset(of: macCatalystKnownFrameworks))
        #expect(!macCatalystKnownFrameworks.isSuperset(of: iOSKnownFrameworks))
    }
}
