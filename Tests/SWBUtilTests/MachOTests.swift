    ...
    fileprivate struct MachOTests {
        ...
        func testMachO(archs: [String], platform: BuildVersion.Platform = .macOS, hideARM64: Bool = false) async throws {
            try await withTemporaryDirectory { (tmpDir: Path) in
                ...
                for file in files {
                    ...
                    let uuids = try Set(
                        machOSlices.compactMap { (slice) -> NSUUID? in
        ...
        func testFatStaticArchives(archs: [String]) async throws {
            try await withTemporaryDirectory { tmpDir in
                ...
                for file in files {
                    ...
                    let uuids = try Set(
                        slices.compactMap { (slice) -> NSUUID? in
                            if let uuidString = try slice.uuid()?.description {
                                return NSUUID(uuidString: uuidString)
                            }
                            return nil
        ...
        func errors() throws {
            ...
            do {
                ...
                #expect(
                    "Reading the data from an empty Mach-O should have thrown",
                    performing: { try machO.slices() },
                    throws: { (error) in
                        checkError(error)
                    }
                )
            ...
            do {
                ...
                #expect(
                    "Reading the data from an empty fat-archive should have thrown",
                    performing: { try machO.slices() },
                    throws: { (error) in
                        checkError(error)
                    }
