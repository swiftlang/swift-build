...
fileprivate struct CohortArchTests: CoreBasedTests {
    ...
    /// DocC depends on a symbol graph for exactly the archs that are extracted: only base archs when cohorts are folded, all archs when built separately.
    @Test(.requireSDKs(.iOS))
    func testCohortArchClangSymbolGraphForDocumentation() async throws {
        try await withTemporaryDirectory(fs: localFS) { (tmpDir: NamedTemporaryDirectory) in
            let core = try await makeCore(tmpDir)
            let doccToolPath = try await self.doccToolPath

            let docTargetName = "ObjCDocFramework"
            let testProject = try await TestProject(
                "aProject",
                groupTree: TestGroup(
                    "SomeFiles",
                    path: "Sources",
                    children: [
                        TestFile("ObjCDocFramework.h"),
                        TestFile("ObjCDocFramework.m"),
                        TestFile("ObjCDocFramework.docc"),
                    ]
                ),
                buildConfigurations: [TestBuildConfiguration(
                    CONFIGURATION,
                    ...
                    TestStandardTarget(
                        ...
                        buildPhases: [
                            TestHeadersBuildPhase([
                                TestBuildFile("ObjCDocFramework.h", headerVisibility: .public),
                            ]),
                            TestSourcesBuildPhase([
                                "ObjCDocFramework.m",
                                "ObjCDocFramework.docc",
                            ]),
                        ]
                    ),
                ]
            ...
            let sourcesFolder = Path(SRCROOT).join("Sources")
            try fs.createDirectory(sourcesFolder, recursive: true)
            try fs.write(sourcesFolder.join("ObjCDocFramework.h"), contents: "@interface ObjCClass\n@end\n")
            try fs.write(sourcesFolder.join("ObjCDocFramework.m"), contents: "@implementation ObjCClass\n@end\n")
            let docsCatalog = sourcesFolder.join("ObjCDocFramework.docc")
            try fs.createDirectory(docsCatalog, recursive: true)
            try fs.write(docsCatalog.join("ObjCDocFramework.md"), contents: "# ``ObjCDocFramework``")
            ...
            func checkSymbolGraphInputs(enableCohortArchs: Bool, expectCohortMemberInput: Bool) async throws {
                ...
                await tester.checkBuild(runDestination: runDestination, buildRequest: request, fs: fs) { results in
                    results.checkTask(.matchRuleItem("CompileDocumentation")) { task in
                        let inputs = task.inputs.map(\.path.str).filter { $0.contains("/symbol-graph/clang/") }
                        #expect(inputs.contains { $0.contains("/symbol-graph/clang/\(baseArch)-") },
                                "Expected a base-arch clang symbol graph input, found: \(inputs)")
                        #expect(inputs.contains { $0.contains("/symbol-graph/clang/\(cohortArch)-") } == expectCohortMemberInput,
                                "Cohort member clang symbol graph input: expected \(expectCohortMemberInput), found: \(inputs)")
                    }
                }
            }
