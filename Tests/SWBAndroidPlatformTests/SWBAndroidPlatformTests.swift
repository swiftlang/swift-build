    ...
    func androidCommandLineTool(arch: String) async throws {
        try await withTemporaryDirectory { (tmpDir: Path) -> () in
            ...
            try await tester.checkBuild(runDestination: destination) { results in
                ...
                results.checkTask(.matchRuleType("Ld"), .matchRuleItemPattern(.suffix(Path("build/Debug-android-undefined_arch/libdynamiclib.so").str))) { task in
                    task.checkCommandLineMatches([
                        ...
                        "-Os",
                        .pathEqual(prefix: "-L", tmpDir.join("build/EagerLinkingTBDs/Debug-android-undefined_arch")),
                        .pathEqual(prefix: "-L", tmpDir.join("build/Debug-android-undefined_arch")),
                    ] + windowsArgs + [
                        .pathEqual(prefix: "@", tmpDir.join("build/TestProject.build/Debug-android-undefined_arch/dynamiclib.build/Objects-normal/\(arch)/dynamiclib.LinkFileList")),
                        "-Xlinker", "-soname", "-Xlinker", "$ORIGIN/libdynamiclib.so",
                    ] + pageSizeFlags + [
                        "-fuse-ld=lld",
                        .and(.prefix("--ld-path="), .contains("ld.lld")),
                        "-o", .path(tmpDir.join("build/Debug-android-undefined_arch/libdynamiclib.so"))
                    ])
                }

                results.checkTask(.matchRuleType("Ld"), .matchRuleItemPattern(.suffix(Path("build/Debug-android-undefined_arch/tool").str))) { task in
                    task.checkCommandLineMatches([
                        .suffix(clang.str),
                        "-target", "\(arch)-unknown-linux-android\(minOS)",
                        ...
                        .pathEqual(prefix: "@", tmpDir.join("build/TestProject.build/Debug-android-undefined_arch/tool.build/Objects-normal/\(arch)/tool.LinkFileList")),
                    ] + pageSizeFlags + [
                        "-fuse-ld=lld",
                        .and(.prefix("--ld-path="), .contains("ld.lld")),
                        "-ldynamiclib",
                        "-lstaticlib",
                        "-o", .path(tmpDir.join("build/Debug-android-undefined_arch/tool"))
                    ])
                }
            ...
            try await tester.checkBuild(runDestination: destination) { results in
                ...
                results.checkTask(.matchRuleType("Ld"), .matchRuleItemPattern(.suffix(Path("build/Debug-android-undefined_arch/libdynamiclib.so").str))) { task in
                    task.checkCommandLineMatches([
                        ...
                        .pathEqual(prefix: "@", tmpDir.join("build/TestProject.build/Debug-android-undefined_arch/dynamiclib.build/Objects-normal/\(arch)/dynamiclib-swiftbuild.autolink")),
                    ] + pageSizeFlags + [
                        "-use-ld=lld",
                        .and(.prefix("-ld-path="), .contains("ld.lld")),
                        "-o", .path(tmpDir.join("build/Debug-android-undefined_arch/libdynamiclib.so"))
                ...
                results.checkTask(.matchRuleType("Ld"), .matchRuleItemPattern(.suffix(Path("build/Debug-android-undefined_arch/tool").str))) { task in
                    task.checkCommandLineMatches([
                        ...
                        .and(.prefix("-L"), .or(.suffix("/usr/lib/swift"), .suffix("\\usr\\lib\\swift\\android"))),
                        .pathEqual(prefix: "-L", Path("/usr/lib/swift")),
                        .pathEqual(prefix: "@", tmpDir.join("build/TestProject.build/Debug-android-undefined_arch/tool.build/Objects-normal/\(arch)/tool-swiftbuild.autolink")),
                    ] + pageSizeFlags + [
                        "-use-ld=lld",
                        .and(.prefix("-ld-path="), .contains("ld.lld")),
                        "-ldynamiclib",
