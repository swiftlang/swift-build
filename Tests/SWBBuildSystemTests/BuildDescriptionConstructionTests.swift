    ...
    func copiedPathMap() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            try await tester.checkBuildDescription(runDestination: .macOS) { results in
                ...
                #expect(
                    results.buildDescription.copiedPathMap == [
        ...
        try await withTemporaryDirectory { tmpDir in
            let workspace = TestWorkspace(
                ...
                projects: [
                    TestPackageProject(
                        "aPackage",
                        groupTree: TestGroup("SomeFiles", children: [TestFile("test.c")]),
                        targets: [
                            TestPackageProductTarget(
                                "PackageLibProduct",
                                frameworksBuildPhase: TestFrameworksBuildPhase([
                            ...
                            TestStandardTarget(
                                "SecondPackageLib",
                                type: .objectFile,
                                buildConfigurations: [TestBuildConfiguration("Debug", buildSettings: ["PRODUCT_NAME": "SecondPackageLib"])],
                                buildPhases: [TestSourcesBuildPhase([TestBuildFile("test.c")])]
                            ),
                        ]
                    ),
                    TestProject(
    ...
    func packageDiamondProblemDiagnosticWithBundleLoader() async throws {
        try await withTemporaryDirectory { tmpDir in
            let workspace = try await TestWorkspace(
                ...
                sourceRoot: tmpDir,
                projects: [
                    TestPackageProject( /* this isn't quite right, but I can't get the diagnostic in a normal target. fix with rdar://79639888 */
                        "aProject",
                        groupTree: TestGroup(
                            "SomeFiles",
                            children: [
