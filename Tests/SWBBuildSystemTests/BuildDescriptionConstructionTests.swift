...
fileprivate struct BuildDescriptionConstructionTests: CoreBasedTests {
    ...
    func copyAndResign() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            try await tester.checkBuildDescription(BuildParameters(action: .install, configuration: "Debug"), signableTargets: Set(provisioningInputs.keys)) { results in
            ...
            try await tester.checkBuildDescription(BuildParameters(action: .install, configuration: "Debug", overrides: ["SDKROOT": "iphoneos"]), signableTargets: Set(provisioningInputs.keys), signableTargetInputs: provisioningInputs) { results in
                // Check that there are no warnings or errors.  In the past we've seen warnings here because not all of the inputs to the re-signing task were being generated by the copy task.
                results.checkNoDiagnostics()
            }
        }
    ...
    func unifdefCopiedPathMap() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            try await tester.checkBuildDescription(BuildParameters(action: .build, configuration: "Debug", overrides: ["COPY_HEADERS_UNIFDEF_FLAGS": "-B"])) { results in
                results.checkNoDiagnostics()
                #expect(results.buildDescription.copiedPathMap == [:])
            }

            try await tester.checkBuildDescription(BuildParameters(action: .build, configuration: "Debug", overrides: ["GCC_OPTIMIZATION_LEVEL": "1"])) { results in
                results.checkNoDiagnostics()
                #expect(results.buildDescription.copiedPathMap == [:])
            }
        }
    ...
    func packageDiamondProblemDiagnosticNoFalsePositives() async throws {
        try await withTesterForPackageDiamondProblemDiagnostic(hasDiamondProblem: false) { tester, tmpDir in
            try await tester.checkBuildDescription(BuildParameters(configuration: "Debug", activeRunDestination: .iOS, arena: arenaInfo(from: tmpDir.join("build")))) { results in
                results.checkNoDiagnostics()
            }
        }
    }
    ...
    func packageDiamondProblemDiagnosticConsidersPackageTargets() async throws {
        try await withTesterForPackageDiamondProblemDiagnosticWithTargets { tester, tmpDir in
            try await tester.checkBuildDescription(BuildParameters(configuration: "Debug", activeRunDestination: .macOS, arena: arenaInfo(from: tmpDir.join("build")))) { results in
    ...
    func packageDiamondProblemDoesNotCrashDuringBuild() async throws {
        try await withTesterForPackageDiamondProblemDiagnostic(offerDynamicVariant: true, requiresMultiPass: true) { tester, tmpDir in
            ...
            let parameters = BuildParameters(action: .build, configuration: "Debug", activeRunDestination: .macOS, arena: arenaInfo(from: tmpDir.join("build")))
            ...
            try await tester.checkBuildDescription(parameters, buildRequest: request) { results in
                results.checkNoDiagnostics()
            }
        }
    }
    ...
    func packageDiamondProblemDoesNotCrashDuringIndexBuild() async throws {
        try await withTesterForPackageDiamondProblemDiagnostic(offerDynamicVariant: true, requiresMultiPass: true) { tester, tmpDir in
            ...
            let parameters = BuildParameters(action: .indexBuild, configuration: "Debug", activeRunDestination: .macOS, arena: arenaInfo(from: tmpDir.join("build")))
    ...
    func packageDiamondProblemDiagnosticTransitive() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            let parameters = BuildParameters(configuration: "Debug", activeRunDestination: .iOS, arena: arenaInfo(from: tmpDir.join("build")))
            let target = try #require(tester.workspace.allTargets.filter({ $0.name == "App" }).first, "could not find app target")
            try await tester.checkBuildDescription(buildRequest: BuildRequest(parameters: parameters, buildTargets: [ BuildRequest.BuildTargetInfo(parameters: parameters, target: target) ], continueBuildingAfterErrors: false, useParallelTargets: true, useImplicitDependencies: true, useDryRun: false)) { results in
    ...
    func packageDiamondProblemDiagnosticDoesNotFlagExecutables() async throws {
        try await withTemporaryDirectory { tmpDir in
            ...
            let tester = try await BuildOperationTester(getCore(), workspace, simulated: true)
            let parameters = BuildParameters(configuration: "Debug", activeRunDestination: .macOS, arena: arenaInfo(from: tmpDir.join("build")))
