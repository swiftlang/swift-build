        ...
        try await withTemporaryDirectory { tmpDirPath async throws -> Void in
                        ...
                        targets: [
                            TestStandardTarget(
                                ...
                                buildPhases: [
                                    ...
                                    TestShellScriptBuildPhase(name: "MyScript", originalObjectID: "", outputs: [tmpDirPath.join("foo.txt").str])
                                ]),
                        ])
                ])
            ...
            do {
                ...
                try await tester.checkBuild(runDestination: .host, buildRequest: modifiedBuildRequest, persistent: true) { results in
                    ...
                    results.checkTask(.matchTargetName("TargetFoo"), .matchRuleType("PhaseScriptExecution")) { task in
                        results.checkBacktrace(task, [
                            "<category='ruleSignatureChanged' description='signature of 'Run custom shell script 'MyScript'' changed: environment changed [inserted 'GCC_PREPROCESSOR_DEFINITIONS=MY_DEF']'>"
                        ])
                    }
                }
            }
            ...
            do {
                ...
                try await tester.checkBuild(runDestination: .host, buildRequest: modifiedBuildRequest, persistent: true) { results in
                    ...
                    results.checkTask(.matchTargetName("TargetFoo"), .matchRuleType("PhaseScriptExecution")) { task in
                        results.checkBacktrace(task, [
                            "<category='ruleSignatureChanged' description='signature of 'Run custom shell script 'MyScript'' changed: environment changed [inserted 'SWIFT_ACTIVE_COMPILATION_CONDITIONS=MY_SWIFT_DEF']'>"
                        ])
                    }
                }
            }
