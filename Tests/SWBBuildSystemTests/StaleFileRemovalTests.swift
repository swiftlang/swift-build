...
fileprivate struct StaleFileRemovalTests: CoreBasedTests {
    ...
    func crossWorkspaceStaleEagerLinkingTBDRemoval() async throws {
        try await withTemporaryDirectory { tmpDirPath in
            ...
            // Workspace B: static library, same build dir, different workspace/build.db.
            // llbuild's delta based SFR can't see workspace A's stale TBDs.
    ...
    /// Test that scoped SFR does not delete TBDs from other configurations.
    @Test(.requireSDKs(.macOS))
    func scopedSFRDoesNotDeleteOtherConfigurations() async throws {
        try await withTemporaryDirectory { tmpDirPath in
            let sharedBuildDir = tmpDirPath.join("SharedBuild")

            func sharedSettings(_ config: String) -> [String: String] {
                return [
                    "GENERATE_INFOPLIST_FILE": "YES",
                    "PRODUCT_NAME": "$(TARGET_NAME)",
                    "OBJROOT": sharedBuildDir.join("Intermediates.noindex").str,
            ...
            let projectA = TestProject(
                ...
                buildConfigurations: [TestBuildConfiguration("Debug", buildSettings: sharedSettings("Debug"))],
                targets: [
                    TestStandardTarget(
                        "Fwk", type: .framework,
            ...
            // Scoped SFR should only scan EagerLinkingTBDs/Release/, leaving Debug/ alone.
            let projectB = TestProject(
                "ProjectB",
                sourceRoot: tmpDirPath.join("SharedSrc"),
                groupTree: TestGroup(
                ...
                buildConfigurations: [TestBuildConfiguration("Release", buildSettings: sharedSettings("Release"))],
                targets: [
                    TestStandardTarget(
                        "Fwk", type: .framework,
                        buildPhases: [TestSourcesBuildPhase(["Fwk.c"])]
            ...
            // Debug TBDs should survive — workspace B only scans Release/.
            let debugTBDsAfterRelease = try localFS.traverse(eagerTBDDir) { path -> Path? in
                if !localFS.isDirectory(path) && path.str.contains("/Debug/") { return path }
                return nil
            }
            #expect(!debugTBDsAfterRelease.isEmpty, "Debug EagerLinkingTBDs should survive after workspace B's Release build because scoped SFR is configuration-aware")
        }
    }
}
